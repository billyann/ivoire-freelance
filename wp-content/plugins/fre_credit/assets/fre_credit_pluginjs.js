		...
		setupData: function(data){
			...
			if (data.paymentType == 'frecredit') {
				...
				if(align) {
					...
					var balance 	= 	fre_credit_globals.currency.icon + parseFloat(fre_credit_globals.available_of_user).toFixed(2);
				}else {
					var price       =   parseFloat(packages.et_price).toFixed(2) + fre_credit_globals.currency.icon;
					var balance 	=  	parseFloat(fre_credit_globals.available_of_user).toFixed(2) + fre_credit_globals.currency.icon;
				}
				this.$el.find('span.available_balance').html( balance );
				...
				Views.historyControl = Views.BlockControl.extend({
					...
					historyFilter: function(event){
						...
						var $target = $(event.currentTarget),
							name = 'history_type',
							view = this;
						$('.function-filter .dropbox span').html($target.attr('data-name'));
						if (name !== 'undefined') {
	                ...
	                onAfterFetch: function(result, res){
	                    if( !res.success || result.length == 0){
	                        $('.fre-credit-history-wrapper').find('.list-histories').html(fre_credit_globals.no_transaction_msg);
	                    }
	                }
				});
