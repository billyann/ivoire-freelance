...
class AE_Milestone_Actions extends AE_Base
{
	...
	public function init() {
		...
		$this->add_filter( 'fre_notify_item', 'ae_render_milestone_notification', 10, 2 );
	...
	public function ae_add_changelog( $milestone_id, $project_id, $action ) {
		...
		$comment_content = $action . ' ' . $milestone->post_title;
		$args = array(
		...
		$comment = new AE_Comments( 'message', array('changed_milestone_id' => '', 'action' => '', 'changelog' => '') );
	...
	public function ae_render_milestone_notification( $content, $notify ) {
		...
		$content = ae_milestone_notification_template( $type, $milestone, $project, $content );
