(function($, Models, Collections, Views) {
    jQuery(document).ready(function($) {
        ...
        if($('.bid-item').hasClass('bid-of-user')){
            ...
            new User_ListBids({
             ...
             new Views.BlockControl({
                ...
                onBeforeFetch: function(){
                    if($('.list-user-bids').find('.no-results').length > 0 ){
                        $('.list-user-bids').find('.no-results').remove();
                    }
                },
                onAfterFetch: function(result, res){
                    if( !res.success || result.length == 0){
                        $('.list-user-bids').find('.list-user-bid-container').html(ae_globals.text_message.no_project);
                    }
                }
            });
        }
        if($('.list-bid-history').length > 0){
            ...
            new Views.BlockControl({
                ...
                el: $('.list-bid-history'),
                onBeforeFetch: function(){
                    if($('.list-bid-history').find('.no-results').length > 0 ){
                        $('.list-bid-history').find('.no-results').remove();
                    }
                },
                onAfterFetch: function(result, res){
                    $.fn.trimContentMobile();
                    if( !res.success || result.length == 0){
                        $('.list-bid-history').find('.list-history-profile').html(ae_globals.text_message.no_work_history);
                    }
                }
            });
        }
        ...
        */
        ...
        $('.more-info-employer, .less-info-employer').on('click', function() {
           $('.info-bid-employer').toggleClass('active');
        });
        $('.more-info-freelancer, .less-info-freelancer').on('click', function() {
           $('.info-bid-freelancer').toggleClass('active');
        });
    });
...
(function($, Views, Models, Collections) {
    ...
    $.fn.trimContentMobile = function() {
        var showChar = 90;  // How many characters are shown by default
        var ellipsestext = "...";
        $('.comment-stt-project p, .comment-author-history p').each(function() {
   ...
   $.fn.trimContentMobile();
   $.fn.showHideReviewMobile = function() {
