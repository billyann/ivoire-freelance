    ...
    if( $post->post_status == 'complete' && ae_get_option('use_escrow') ) {
            ...
            if(ae_get_option('manual_transfer', false)){
                if($order_status != 'finish') :?>
                    <span class="btn btn-project-status btn-excecute-project manual-transfer"  title="<?php _e("Transfer Money To Freelancer", ET_DOMAIN); ?>">
        ...
        <?php if($post->post_status == 'disputing'){ ?>
            <a class="btn btn-excecute-project btn-agree btn-arbitrate-project" href="#"><?php _e("Arbitrate", ET_DOMAIN); ?></a>
        <?php }else{ ?>
            <ul class="button-event event-listing">
                ...
                <?php if(in_array($post->post_status, array('pending', 'publish', 'reject', 'draft'))){ ?>
                    <li class="tooltips update edit">
                        <a class="action" data-action="edit" data-toggle="tooltip" title="<?php _e("Edit", ET_DOMAIN); ?>" data-original-title="<?php _e("Edit", ET_DOMAIN); ?>" href="#"><i class="fa fa-pencil" data-icon="p"></i></a>
                    </li>
                <?php } ?>
                <!-- button edit -->
                <!-- button archive -->
                <?php if(in_array($post->post_status, array('publish', 'reject', 'draft'))){ ?>
