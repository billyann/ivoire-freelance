    ...
    <div class="step-content-wrapper content">
        ...
        <?php foreach ($packs as $key => $package) {
            ...
            if($number_of_post >= 1 ) {
                ...
                if( !$package->et_price ) {
                    $number_of_post = $number_of_post - $number_free_plan_used;

                    if($number_of_post < 0) $number_of_post = 0;
                }
                if($number_of_post > 0 ) {
                    if($number_of_post > 1){
                        $text = sprintf(__("You can submit <b>%d posts</b> using this plan.", ET_DOMAIN) , $number_of_post);
                    }else{
                        $text = sprintf(__("You can submit <b>%d post</b> using this plan.", ET_DOMAIN) , $number_of_post);
                    }
                }else {
                    $text = sprintf(__("You can submit <b>%d posts</b> using this plan.", ET_DOMAIN) , $number_of_post);
                }
        ...
        ?>
        	<li <?php echo $class_select; ?> data-sku="<?php echo $package->sku ?>" data-id="<?php echo $package->ID ?>" data-price="<?php echo $package->et_price; ?>"
             ...
             >
                ...
                <?php if(!$package->et_price && $number_of_post == 0) :?>
                    <a href="#" class="btn btn-submit-price-plan select-plan disable" disabled data-toggle="popover" data-content="<?php _e('Your free package is already in use', ET_DOMAIN); ?>" data-target="#"><?php _e( 'Select' , ET_DOMAIN ); ?></a>
                <?php else:?>
