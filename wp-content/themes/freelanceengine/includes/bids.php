...
class Fre_BidAction extends AE_PostAction
{
    ...
    public function fre_notify_admin_bid($data){
        if(!isset($data['order_id'])) return false;
    ...
    function ae_convert_bid($result) {
        ...
        $result->bid_budget_text = fre_price_format($result->bid_budget, 'sup');
    ...
    function ae_convert_check_bid_item($result){
        ...
        if($role == FREELANCER){
            ...
            if(!(in_array($project_status, array('complete','close','disputing','disputed' )) && $bid_accept == $result->ID) &&  $project_status != 'publish'){
                $hide = 'bid_unaccepted bid_hide';
            }
        }else{
            // hide all
        ...
        if( $user_ID == (int) $result->project_author && $project_status == 'publish' ){
            $flag = 1;
        }else if( $bid_accept && $bid_accept == $result->ID && in_array($project_status, array('complete','close','disputing','disputed' ) ) ){
            $flag = 2;
        }
        $class = $hide.' '.$user_bid;
