...
class Fre_ProjectAction extends AE_PostAction
{
    function __construct($post_type = 'project') {
        ...
        $this->add_filter('ae_reject_post_message', 'replace_mail_reject_post');
    }
    /**
    ...
    */
    ...
    public function mail_after_process_payment_by_admin($id){
        ...
        $project_obj = $ae_post_factory->get('project');
        $convert = $project_obj->convert($post);
        if($convert->post_status == 'publish' && (is_super_admin() || !ae_get_option('use_pending', false))){
            $this->mail->new_project_of_category($convert);
        }
    }
    ...
    public function mail_after_change_status($post){
        ...
        $project_obj = $ae_post_factory->get('project');
        $convert = $project_obj->convert($post);
        if(isset($request['method']) && $request['method'] == 'update' && $post->post_status == 'publish'){
            $this->mail->new_project_of_category($convert);
        }
    }
    ...
    public function filter_query_args($query_args) {
        ...
        if (isset($query['is_profile']) && $query['is_profile'] == 'true') {
            ...
            if (!isset($query['post_status']) || $query['post_status'] == '') {
                $query_args['post_status'] = array(
                    'pending','publish','close', 'complete', 'disputing', 'disputed', 'archive', 'reject', 'draft'
                );
            }else{
    ...
    function post_sync() {
        ...
        if (!isset($request['archive'])) {
            ...
            if(ae_get_option('gg_captcha', false) && ae_get_option('gg_secret_key') && !current_user_can( 'manage_options' ) &&
                !isset($request['captcha_complete']) ){
    ...
    function ae_convert_project($result) {
        ...
        if ($result->post_status == 'disputing') $result->status_text = __("DISPUTED", ET_DOMAIN);

        //project is diputed
        if ($result->post_status == 'disputed') $result->status_text = __("RESOLVED", ET_DOMAIN);
...
function list_tax_of_project($id, $title = '', $taxonomy = 'project_category', $class = '') {
    ...
    if ($terms && !is_wp_error($terms)): ?>
        <h3 class="title-content"><?php printf($title); ?></h3>
        <div class="list-require-skill-project list-taxonomires list-<?php echo $taxonomy; ?>">
            <?php the_taxonomy_list($taxonomy, '<span class="skill-name-profile">', '</span>', $class); ?>
        </div>
        <?php
    endif;
}
        ...
        <?php } else { ?>
                <li>
                    ...
                    <span class="member-wrap">
                        <span class="title-name"><i class="fa fa-map-marker"></i><?php _e('Member Sinced:', ET_DOMAIN); ?></span>
...
function fre_sub_taxonomy_dropdow($taxonomy,$args){
    ...
    foreach ($terms as $term) {
        if($term->parent == 0){
            if($value_type == 'slug') {
                echo "<option value='".$term->slug."' class='".$term->slug." cat-".$tern_id."'>";
            } elseif($value_type == 'id') {
                echo "<option value='".$term->term_id."' class='".$term->slug." cat-".$tern_id."'>";
            }
            echo $term->name;
            echo "</option>";
            foreach ($terms as $value) {
                if($term->term_id == $value->parent){
                    if($value_type == 'slug') {
                        echo "<option value='".$value->slug."' class='".$term->slug." cat-".$tern_id."'>";
                    } elseif($value_type == 'id') {
                        echo "<option value='".$value->term_id."' class='".$term->slug." cat-".$tern_id."'>";
                    }
                        echo "--".$value->name;
                    echo "</option>";
                }
