...
class Fre_Notification extends AE_PostAction
{
    ...
    function fre_notify_item($notify) {
        ...
        switch ($type) {
            case 'close_project':
                ...
                $content.= sprintf(__("%s closed your working project %s", ET_DOMAIN), $author , $project_link);
                break;
            case 'quit_project':
                $project_owner = get_post_field('post_author', $project);
                ...
                $content.= sprintf(__("%s discontinued on your project %s", ET_DOMAIN) ,$author, $project_link);
                break;
            case 'delete_bid':
                // get bid author
                ...
                $user_profile_id = get_user_meta($freelancer, 'user_profile_id', true);

                // render bid author data
                $author = '<a class="user-link" href="'. get_author_posts_url($freelancer) .'">
                        <span class="avatar-freelancer-notification">
                            ' . get_avatar($freelancer, 45) . '
                        </span>
                        <div class="profile">
                            <span class="name">
                                ' . get_the_author_meta('display_name', $freelancer) . '
                            </span>
                            <span class="position-employer">
                                ' . get_post_meta($user_profile_id, 'et_professional_title', true) . '
                            </span>
                        </div>
                        </a>';

                $content.= sprintf(__("%s cancelled his bid on your project %s", ET_DOMAIN) , $author, $project_link);
                break;
            ...
            case 'new_bid':
                ...
                $content.= sprintf(__("%s bidded for your project %s", ET_DOMAIN) , $author, $project_link);
                break;
                   // notify employer when his project have new bid
            // case 'new_project':
            ...
            case 'review_project':
                ...
                $content.= sprintf(__("%s reviewed on %s", ET_DOMAIN) , $author, $project_link);
                break;
            ...
            case 'new_comment':
                if($comment_author_id)
                    $author = '<a class="user-link" href="'. get_author_posts_url($comment_author_id) .'" ><span class="avatar-notification">
                                ' . get_avatar($comment_author_id, 30) . '
                            </span>&nbsp;&nbsp;
                            <span class="date-notification">
                    ...
                    $author = '<span class="avatar-notification">' . get_avatar($comment_author_id, 30) . '</span>&nbsp;&nbsp;
                            <span class="date-notification">
                                ' . $comment_author . '
                            </span>
                            </a>';
                $content.= sprintf(__("%s has comment on %s", ET_DOMAIN) , $author, $project_link);
                break;
    ...
    function fre_approve_comment_callback($new_status, $old_status, $comment) {
        ...
        if(!in_array($post_status, array('pending', 'publish'))) return;
