...
function fre_new_ae_global($vars){
    ...
    $vars['switch_skill'] = ae_get_option('switch_skill', false);
    $vars['gg_captcha'] = ae_get_option('gg_captcha', false);
    ...
    $vars['fre_updated_bids']  = ae_get_option('fre_updated_bids', 0);
    $vars['fre_updated_bid_accept']  = ae_get_option('fre_updated_bid_accept', 0);
    $vars['text_view'] = array('more' => __('View more', ET_DOMAIN), 'less' => __('View less', ET_DOMAIN));
    $vars['text_message'] = array(
            'no_project'        => __("<span class='no-results'>There are no projects found.</span>", ET_DOMAIN),
...
function fre_button_bid($project_id){
    ...
    $target     = '#modal_bid';
    $class_bid  = '';
    $profileText =  __('You must complete your profile to bid on a project.', ET_DOMAIN);
    $inviteText =  __('You must be invited to bid this project.', ET_DOMAIN);
    $noBidText = __("You don't have enough available bid(s) for project bidding.", ET_DOMAIN);
    if(et_load_mobile()){
        $class  = 'btn btn-apply-project-item btn-bid btn-agree';
        $class_bid = 'btn-bid-mobile';
    }

    // user have to complete profile to bid a project
    if( !$profile || !is_numeric($profile_id) ) {
        echo '<a href="#" class="'.$class.' not-have-bid " data-toggle="popover" data-content="'. $profileText.'">'.__('Bid',ET_DOMAIN).'</a>';
    }else{ 
        // Check invite to bid
        if( ae_get_option('invited_to_bid') && !fre_check_invited($user_ID, $project_id) ) {
            echo '<a class="'.$class.' not-have-bid " href="#" data-toggle="popover" data-content="'.$inviteText.'">'.__('Bid', ET_DOMAIN).'</a>';
        }else{
            // disable option pay to bid
            if( !can_user_bid( $user_ID ) ){
                $href = et_get_page_link('upgrade-account', array('project_id' => $project_id));
                echo '<a href="'.$href.'" class="btn btn-purchase-project-item btn-login-trigger">'.__('Purchase Bid', ET_DOMAIN).'</a>';
                echo '<a href="#" class="'.$class.'  not-have-bid"  data-toggle="popover" data-content="'.$noBidText.'">'.__('Bid', ET_DOMAIN).'</a>';
            }else{
                echo '<a href="#" class="'.$class.' '.$class_bid.'" data-toggle="modal" data-target="'.$target.'">'.__('Bid', ET_DOMAIN).'</a>';
            }
        }
    }
}
