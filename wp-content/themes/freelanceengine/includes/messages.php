        ...
        if(!empty($file_arr) && is_array($file_arr)) {
            ...
            foreach ($attachment as $key => $value) {
                ...
                if($post->post_status == 'close' && $value->post_author == $user_ID && !$value->post_parent){
                    $html_removeAtt = '<a href="#" data-id="'.$value->ID.'" data-project="'.$post->ID .'" data-filename="'.$value->post_title.'" class="removeAtt"><i class="fa fa-times" aria-hidden="true"></i></a>';
                }else{
                    $html_removeAtt = '';
                }
...
class Fre_MessageAction extends AE_Base
{
    ...
    function preventAccessWorkspace(){
        ...
        if(is_singular( PROJECT )){
            ...
            // check project owner
            $project = $post;
            if(current_user_can('manage_options') && $user_ID != $project->post_author) return;
            // check freelancer was accepted on project
            $bid_id = get_post_meta($project->ID, "accepted", true);
            $bid = get_post($bid_id);
            
            // current user is not project owner, or working on
            if ( in_array($project->post_status, array('disputing', 'disputed')) && (($user_ID != $project->post_author && $user_ID != $bid->post_author) || !is_user_logged_in())) {
                wp_redirect( 404 );
                exit;
            }            
        }
    ...
    function addWorkSpaceLink($project){
        ...
        if($permission){
            ...
            echo '<a class="workspace-link"  style="font-weight:600;" href='.add_query_arg(array('workspace' => 1), $project_link).'>'.__("Open Workspace", ET_DOMAIN).'</a>';
        }
    }
    /**
