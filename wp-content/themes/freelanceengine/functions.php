...
class ET_FreelanceEngine extends AE_Base
{
    ...
    function restrict_pages() {
        ...
        if (is_page_template('page-submit-project.php')) {
            ...
            if(!is_user_logged_in() && !fre_check_register()){
                wp_redirect(home_url());
                exit;
            }
        }
        // $re = (fre_check_register()) ? true : false;
        // if(!$re){
        //     if(!is_user_logged_in() && is_page_template('page-submit-project.php')){
        //         wp_redirect(home_url());
        //         return;
        //     }
        // }
    ...
    */
    ...
    function filter_link_redirect_register($result) {
        ...
        if( $role == FREELANCER){
            if( et_load_mobile() ){
                $redirect_url = et_get_page_link('profile', array('loginfirst' => 'true')).'#tab_profile';
            } else{
